...
class GalleryFilterNetwork(nn.Module):
    def __init__(self,emb_dim=256, temp=0.1, se_temp=0.2,
            filter_neg=True, gfn_query_mode='batch',
            gfn_activation_mode='se', pos_num_sample=1, neg_num_sample=1,
    ...
    def forward(self, scene_emb, query_emb, targets):
        # Produce labels
        ...
        image_labels = torch.LongTensor(gfn_image_labels).to(self.device)   # tensor([0, 1, 2, 3, 4, 5, 6, 7...], device='cuda:0')
        ...
        # Boxvar
        # gfn_lut_label_list = [t['labels']-1 for t in targets]   # memory中的位置
        # Boxes
        # boxes = gt_boxes
        # Labels
        ...
        # query_features = self.query_roi_pool(features, boxes, image_shapes) # ([23, 512, 14, 14])
        ...
        # scene_emb = self.head(features) # [8, 2048]
        I = scene_emb.size(0)
