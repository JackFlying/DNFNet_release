...
from mmdet.models.utils import HybridMemoryMultiFocalPercent, Quaduplet2Loss, ClusterMemory


@HEADS.register_module()
class HHCLHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss_reid = ClusterMemory(num_features=num_features, num_samples=0, \
                                        temp=temperature, momentum=momentum, mode='CMhybrid', \
                                        smooth=smooth, num_memory=id_num)
        self.no_bg = no_bg
    ...
    def loss(self,
        ...
        # -2:nothing, -1:bg
        rid_pred = id_pred[id_labels >= 0]
        cluster_id_labels = self.loss_reid.get_cluster_ids(id_labels[id_labels >= 0])
        losses['loss_id'] = self.loss_reid(rid_pred, cluster_id_labels)
