...
class DNFNetSiameseHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(DNFNetSiameseHead, self).__init__()
        ...
        # self.gt_feature = nn.Linear(in_channels, 128)
        # self.gt_feature1 = nn.Linear(in_channels // 2, 128)
    ...
    def init_weights(self):
        ...
        nn.init.normal_(self.id_part_feature.weight, 0, 0.001)
        nn.init.constant_(self.id_part_feature.bias, 0)
        nn.init.normal_(self.id_part_feature1.weight, 0, 0.001)
        nn.init.constant_(self.id_part_feature1.bias, 0)

        # nn.init.normal_(self.gt_feature.weight, 0, 0.001)
        # nn.init.constant_(self.gt_feature.bias, 0)
        # nn.init.normal_(self.gt_feature1.weight, 0, 0.001)
        # nn.init.constant_(self.gt_feature1.bias, 0)


    @auto_fp16()
    def forward(self, bbox_feats, bbox_feats1, part_feats_list, part_feats1_list, gt_feats_list, gt_feats1_list):
        """
            x: [N, 1024, 14, 6]
            gt_x: [N, 1024, 14, 6]
        ...
        bbox_feats1 = bbox_feats1.view(bbox_feats1.size(0), -1)
        cls_score = self.fc_cls(bbox_feats) if self.with_cls else None
        bbox_pred = self.fc_reg(bbox_feats) if self.with_reg else None

        id_feat1, id_feat2 = self.id_feature(bbox_feats), self.id_feature1(bbox_feats1)
        id_pred = F.normalize(torch.cat((id_feat1, id_feat2), axis=1))

        id_pred_part_list = None
        if part_feats_list is not None:
            id_pred_part_list = []
            for i, (x, x1) in enumerate(zip(part_feats_list, part_feats1_list)):
                x, x1 = x.view(x.size(0), -1), x1.view(x1.size(0), -1)
                x, x1 = self.id_part_feature(x), self.id_part_feature1(x1)
                x_cat = F.normalize(torch.cat((x, x1), axis=1))
                id_pred_part_list.append(x_cat)

        gt_id_pred_list = []
        for i, (x, x1) in enumerate(zip(gt_feats_list, gt_feats1_list)):
            x, x1 = x.view(x.size(0), -1), x1.view(x1.size(0), -1)
            if i == 0:  # for global
                x, x1 = self.id_feature(x), self.id_feature1(x1)
            else:
                x, x1 = self.id_part_feature(x), self.id_part_feature1(x1)
            x_cat = F.normalize(torch.cat((x, x1), axis=1))
            gt_id_pred_list.append(x_cat)

        if not self.training:
            if gt_id_pred_list is not None:
                # gt + pred, 特征需要相同的分布,所以用同一个mlp提取特征
