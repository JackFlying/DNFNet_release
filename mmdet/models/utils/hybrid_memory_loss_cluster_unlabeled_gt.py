...
def hm_part(inputs, bottom_inputs, top_inputs, indexes, labels, cluster_mean, cluster_std, features, bottom_features, top_features, mIoU, momentum, \
    ...
    return HM_part.apply(
        ...
        torch.Tensor([momentum]).to(inputs.device), torch.Tensor(IoU_memory_clip).to(inputs.device), 
...
class HM(autograd.Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, inputs, gt_feats, indexes, labels, features, cluster_mean, tflag, IoU, update_method, momentum, update_flag, IoU_memory_clip, targets, \
                        clock, cluster_mean_method, tc_winsize, intra_cluster_T):
        ctx.features = features # memory特征
        ctx.gt_feats = gt_feats
        ctx.cluster_mean = cluster_mean
        ctx.labels = labels
        ctx.momentum = momentum
        ctx.update_method = update_method
        ...
        all_gt_feats = all_gather_tensor(gt_feats)
        ctx.save_for_backward(all_inputs, all_gt_feats, all_indexes, all_IoU, all_tflag)
    ...
    def backward(ctx, grad_outputs):
        """
            ctx.update_flag: max iou flag
        """
        inputs, gt_inputs, indexes, IoU, tflag = ctx.saved_tensors
        ...
        for x, xg, y, iou, uf, tg in zip(inputs, gt_inputs, indexes, IoU, ctx.update_flag, ctx.targets):
            if ctx.update_method == "momentum":
                ctx.features[y] = ctx.momentum * ctx.features[y] + (1.0 - ctx.momentum) * x
            elif ctx.update_method == "iou":
                ctx.features[y] = (1 - iou) * ctx.features[y] + iou * x
...
def hm(inputs, gt_feats, indexes, labels, features, cluster_mean, tflag, IoU, update_method=None, momentum=0.5, update_flag=None, IoU_memory_clip=0.2, \
            targets=None, clock=0, cluster_mean_method='naive', tc_winsize=500, intra_cluster_T=0.1):
    return HM.apply(
        inputs, gt_feats, indexes, labels, features, cluster_mean, tflag, IoU, update_method, torch.Tensor([momentum]).to(inputs.device), update_flag, \
            torch.Tensor(IoU_memory_clip).to(inputs.device), targets, clock, cluster_mean_method, tc_winsize, intra_cluster_T,
    )

class HybridMemoryMultiFocalPercentClusterUnlabeledGt(nn.Module):
    ...
    def __init__(self, num_features, num_memory, temp=0.05, momentum=0.2, cluster_top_percent=0.1, instance_top_percent=1, \
        ...
        super(HybridMemoryMultiFocalPercentClusterUnlabeledGt, self).__init__()
    ...
    def forward(self, feats, gt_feats, indexes, IoU, top_IoU, bottom_IoU, part_feats=None):
        """
            :feats: [B, 256]
            :indexes: [B, ]
            :IoU: [B, ]
        ...
        if self.use_part_feat:
            ...
            inputs = hm(feats, gt_feats, indexes, labels, self.features, self.cluster_mean, self.tflag, IoU, self.update_method, self.momentum, update_flag, self.IoU_memory_clip, \
