...
class HM(autograd.Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, inputs, inputs_log_var, indexes, labels, features, features_std, cluster_mean, cluster_std, IoU, update_method, momentum, update_flag, IoU_memory_clip, targets, sample_times):
        ...
        for i in range(sample_times):
            ...
            # inputs_ = F.normalize(inputs)
            outputs.append(inputs.mm(center_sample.t()))
        ...
        ctx.save_for_backward(all_inputs, all_inputs_log_var, all_indexes, all_IoU)
        return outputs

    @staticmethod
    @custom_bwd
    def backward(ctx, grad_outputs):
        inputs, inputs_log_var, indexes, IoU = ctx.saved_tensors
        grad_inputs = None
        if ctx.needs_input_grad[0]:
            grad_inputs = []
        ...
        for x, xlv, y, iou, uf, tg in zip(inputs, inputs_log_var, indexes, IoU, ctx.update_flag, ctx.targets):
            if ctx.update_method == "momentum":
                ctx.features[y] = ctx.momentum * ctx.features[y] + (1.0 - ctx.momentum) * x
            elif ctx.update_method == "iou":
                ctx.features[y] = (1 - iou) * ctx.features[y] + iou * x
...
def hm(inputs, id_pred_log_var, indexes, labels, features, features_std, cluster_mean, cluster_std, IoU, update_method=None, momentum=0.5, update_flag=None, IoU_memory_clip=0.2, targets=None, sample_times=1):
    return HM.apply(
        inputs, id_pred_log_var, indexes, labels, features, features_std, cluster_mean, cluster_std, IoU, update_method, torch.Tensor([momentum]).to(inputs.device), update_flag, \
            torch.Tensor(IoU_memory_clip).to(inputs.device), targets, sample_times
    )

class HybridMemoryMultiFocalPercentMCDropout(nn.Module):
    ...
    def __init__(self, num_features, num_memory, temp=0.05, momentum=0.2, cluster_top_percent=0.1, instance_top_percent=1, \
        ...
        super(HybridMemoryMultiFocalPercentMCDropout, self).__init__()
    ...
    def forward(self, feats, indexes, IoU, part_feats, top_IoU, bottom_IoU, pos_is_gt_list, id_pred_log_var):
        ...
        var = ((feats - mean[:, None])**2).sum(dim=1) / feats.shape[1]
        ...
        if self.use_part_feat:
            ...
            inputs = hm(mean, var, indexes, labels, self.features, self.features_std, self.cluster_mean, self.cluster_std, IoU, self.update_method, self.momentum, update_flag, self.IoU_memory_clip, \
        ...
        if self.use_cluster_hard_loss:
            ...
            if targets.shape[0] > 0:
                ...
                for k in range(0, self.sample_times):
