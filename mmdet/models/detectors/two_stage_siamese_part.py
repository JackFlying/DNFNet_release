...
class TwoStageDetectorSiamesePart(BaseDetector):
    ...
    def __init__(self,
        ...
        super(TwoStageDetectorSiamesePart, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        Args:
            pretrained (str, optional): Path to pre-trained weights.
                Defaults to None.
        """
        super(TwoStageDetectorSiamesePart, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        top_gt_bboxes_list, bottom_gt_bboxes_list = [], []
        if self.with_bbox or self.with_mask:
            ...
            for i in range(num_imgs):
                ...
                # 获取top和bottom的proposals
                top_gt_bboxes = gt_bboxes[i].clone()   # [x1, y1, x2, y2]
                top_gt_bboxes[:, 1] = (top_gt_bboxes[:, 1] + top_gt_bboxes[:, 3]) / 2   # [x1, (y1+y2)/2, x2, y2]
                bottom_gt_bboxes = gt_bboxes[i].clone()
                bottom_gt_bboxes[:, 3] = (bottom_gt_bboxes[:, 1] + bottom_gt_bboxes[:, 3]) / 2  # [x1, y1, x2, (y1+y2)/2]
                top_gt_bboxes_list.append(top_gt_bboxes)
                bottom_gt_bboxes_list.append(bottom_gt_bboxes)
        ...
        if self.use_mask:
            ...
            gt_bottom_img = self.gt_align(img, bottom_gt_bboxes_list)
            gt_img_list = [gt_img, gt_top_img, gt_bottom_img]

        gt_x_list = [self.extract_feat(x) for x in gt_img_list]
        roi_losses = self.roi_head.forward_train(x, gt_x_list, img_metas, sampling_results,
                                                 gt_bboxes, gt_labels,
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False, **kwargs):
        ...
        # 提取part特征
        bottom_proposal = proposal_list[0].clone()
        bottom_proposal[:, 3] = (bottom_proposal[:, 1] + bottom_proposal[:, 3]) / 2  # [x1, y1, x2, (y1+y2)/2]
        top_proposal = proposal_list[0].clone()   # [x1, y1, x2, y2]
        ...
        if proposal_list[0].shape[1] == 5:
            ...
            bottom_proposal_img = self.gt_align(img, [bottom_proposal[:, :-1]])
            top_proposal_img = self.gt_align(img, [top_proposal[:, :-1]])
        if proposal_list[0].shape[1] == 4:
            proposal_img = self.gt_align(img, [proposal_list[0]])
            bottom_proposal_img = self.gt_align(img, [bottom_proposal])
            top_proposal_img = self.gt_align(img, [top_proposal])
            
        proposal_img_list = [proposal_img, bottom_proposal_img, top_proposal_img]
        gt_x_list = [self.extract_feat(x) for x in proposal_img_list]
        
        return self.roi_head.simple_test(
            x, gt_x_list, proposal_list, img_metas, rescale=rescale)
