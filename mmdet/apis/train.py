...
def train_detector(model,
    ...
    if validate:
        if cfg.data_name == 'PRW':
            ...
            query_dataset = build_dataset(cfg.data.test, dict(test_mode=True))
            ...
            query_data_loader = build_dataloader(
                ...
                dist=distributed,
                shuffle=False)
            val_dataloader = [data_loader, query_data_loader]

        eval_cfg = cfg.get('evaluation', {})
        eval_hook = DistEvalHook if distributed else EvalHook
        runner.register_hook(eval_hook(val_dataloader, log=log, cfg=cfg, **eval_cfg))
    ...
    if cfg.get('SPCL', None):
        ...
        runner.register_hook(extract_feature_hook(train_cluster_dataloader, cfg=cfg, logger=logger, pretrained_feature_file=pretrained_feature_file))
