...
# @jit(forceobj=True)
def evaluate_search_prw(all_dets, epoch, log, det_thresh=0.05, gallery_size=-1, ignore_cam_id=True):
    ...
    search_performance_calc(gallery_set, probe_set, gallery_det, gallery_feat, probe_feat, epoch, log, det_thresh, gallery_size, ignore_cam_id)

# @jit(forceobj=True)
def search_performance_calc(gallery_set, probe_set,
                                gallery_det, gallery_feat, probe_feat, epoch, log,
    ...
    log.add_scalar('val/mAP', mAP, epoch)
    accs = np.mean(accs, axis=0)
    for i, k in enumerate(topk):
        print('  top-{:2d} = {:.2%}'.format(k, accs[i]))
        log.add_scalar('val/top-{:2d}'.format(k), accs[i], epoch)
