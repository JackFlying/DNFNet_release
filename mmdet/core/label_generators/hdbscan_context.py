...
__all__ = ["label_generator_hdbscan_context_single", "label_generator_context_hdbscan"]
...
@torch.no_grad()
def label_generator_hdbscan_context_single(cfg, features, dist, eps, **kwargs):
    assert isinstance(dist, np.ndarray)

    # clustering
    min_cluster_size = 2 # the smallest size grouping that you wish to consider a cluster
    min_samples = 8    #  The larger the value of min_samples you provide, the more conservative the clustering – more points will be declared as noise
    use_outliers = True
    
    cluster = HDBSCAN(min_cluster_size=min_cluster_size, min_samples=min_samples, cluster_selection_epsilon=eps, core_dist_n_jobs=-1)
...
@torch.no_grad()
def label_generator_hdbscan_context(cfg, features, cuda=True, indep_thres=None, all_inds=None, **kwargs):
    assert cfg.PSEUDO_LABELS.cluster == "hdbscan_context"
    ...
    if len(eps) == 1:
        # normal clustering
        labels, centers, num_classes = label_generator_hdbscan_context_single(
        ...
        eps = sorted(eps)
        labels_tight, centers_tight, num_classes_tight = label_generator_hdbscan_context_single(cfg, features, dist, eps[0])
        labels_normal, centers_normal, num_classes = label_generator_hdbscan_context_single(
            cfg, features, dist, eps[1]
        )
        labels_loose, centers_loose, num_classes_loose = label_generator_hdbscan_context_single(cfg, features, dist, eps[2])
