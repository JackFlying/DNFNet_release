...
import numpy as np
import torch
from mmdet.core.label_generators.infomap import get_dist_nbr, cluster_by_infomap
from .compute_dist import build_dist

__all__ = ["label_generator_infomap_context_single", "label_generator_context_infomap"]
...
@torch.no_grad()
def label_generator_infomap_context_single(cfg, features, dist, eps, **kwargs):
    assert isinstance(dist, np.ndarray)

    # clustering
    use_outliers = True
    #  --k1 15 --k2 4
    feat_dists, feat_nbrs = get_dist_nbr(features=features.numpy(), k=15, knn_method='faiss-gpu')

    labels = cluster_by_infomap(feat_nbrs, feat_dists, min_sim=eps, cluster_num=4)
...
@torch.no_grad()
def label_generator_infomap_context(cfg, features, cuda=True, indep_thres=None, all_inds=None, **kwargs):
    assert cfg.PSEUDO_LABELS.cluster == "infomap_context"
    ...
    if len(eps) == 1:
        # normal clustering
        labels, centers, num_classes = label_generator_infomap_context_single(
        ...
        eps = sorted(eps)
        labels_tight, centers_tight, num_classes_tight = label_generator_infomap_context_single(cfg, features, dist, eps[0])
        labels_normal, centers_normal, num_classes = label_generator_infomap_context_single(
            cfg, features, dist, eps[1]
        )
        labels_loose, centers_loose, num_classes_loose = label_generator_infomap_context_single(cfg, features, dist, eps[2])
